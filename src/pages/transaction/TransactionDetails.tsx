      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted small">
            شناسه: <span className="dir-ltr d-inline-block text-navy fw-bold">{transaction._id}</span>
          </p>
        </div>
        <button 
          onClick={() => navigate(-1)} 
          ...
          <div className="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
            <div className={`card-header border-0 py-3 d-flex justify-content-between align-items-center bg-${statusInfo.color}-subtle`}>
                <div className="d-flex align-items-center gap-2">
                    ...
                    <span className={`fw-bold text-${statusInfo.color} fs-5`}>وضعیت: {statusInfo.text}</span>
                </div>
            ...
            <div className="card-body p-4 text-center">
                ...
                <h1 className="fw-bolder text-dark mb-0 display-5">
                    {formatNumber(transaction.amount)} <span className="fs-5 text-secondary fw-normal">تومان</span>
                </h1>
                {transaction.transaction_data?.ref_id && transaction.transaction_data.ref_id !== "_" && (
                     <div className="mt-3 bg-light d-inline-block px-3 py-2 rounded-3 border">
                        ...
                        <span className="fw-bold dir-ltr font-monospace text-dark">{transaction.transaction_data.ref_id}</span>
                     </div>
                )}
            </div>
          </div>
                ...
                <div className="row g-4">
                    <div className="col-md-6">
                        <div className="d-flex align-items-center gap-3 p-3 rounded-3 bg-light h-100">
                            <div className="bg-white p-2 rounded-circle shadow-sm text-navy"><User size={20}/></div>
                            <div>
                                <small className="text-muted d-block">نام و نام خانوادگی</small>
                                <span className="fw-bold text-dark">{transaction.userId?.name} {transaction.userId?.family}</span>
                            </div>
                        </div>
                    </div>
                    <div className="col-md-6">
                        <div className="d-flex align-items-center gap-3 p-3 rounded-3 bg-light h-100">
                            <div className="bg-white p-2 rounded-circle shadow-sm text-success"><Smartphone size={20}/></div>
                            <div>
                                <small className="text-muted d-block">شماره موبایل</small>
                                <span className="fw-bold text-dark dir-ltr">{transaction.userId?.mobile}</span>
                            </div>
                        </div>
                    </div>
                    <div className="col-md-6">
                        <div className="d-flex align-items-center gap-3 p-3 rounded-3 bg-light h-100">
                            <div className="bg-white p-2 rounded-circle shadow-sm text-info"><Mail size={20}/></div>
                            <div>
                    ...
                    <div className="col-md-6">
                         <div className="d-flex align-items-center gap-3 p-3 rounded-3 bg-light h-100">
                            <div className="bg-white p-2 rounded-circle shadow-sm text-warning"><CheckCircle size={20}/></div>
                            <div>
                                <small className="text-muted d-block">وضعیت کاربر</small>
                                <span className="fw-bold text-dark">{transaction.userId?.status === 'active' ? 'فعال' : 'غیرفعال'}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="row g-4">
                            ...
                            <div className="bg-navy-subtle p-4 rounded-3 d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <span className="badge bg-navy rounded-pill">
                                        پرداخت: {transaction.planId.typePay === "Year" ? "سالانه" : transaction.planId.typePay}
                                    </span>
                                </div>
                                <div className="text-end">
                                    <small className="text-muted d-block mb-1">قیمت پایه</small>
                                    <h4 className="fw-bold m-0 text-dark">{formatNumber(transaction.planId.price)}</h4>
                                </div>
                            </div>
                        </div>
                        <div className="col-6 col-md-4 text-center">
                            <div className="p-3 border rounded-3 bg-light h-100">
                                ...
                                <div className="fw-bold fs-5">{formatNumber(transaction.planId.limitOfRequest)}</div>
                            </div>
                        </div>
                        <div className="col-6 col-md-4 text-center">
                            <div className="p-3 border rounded-3 bg-light h-100">
