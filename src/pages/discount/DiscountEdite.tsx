// src/pages/discount/DiscountEdit.tsx
...
import PlanSelectorModal, {
  type Plan,
} from "../../components/PlanSelectorModal";
import UserSelectorModal, {
  type User,
} from "../../components/UserSelectorModal";

// --- Interfaces (Matched with Details/Create) ---
...
const DiscountEdit: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDiscount = async () => {
      ...
      try {
        ...
        const data: DiscountData = response.data.discount || response.data;

        // Populate States
        setTitle(data.title);
        setAmount(data.amount.toString());

        // Handle Type Mapping (API might return 'amount', UI uses 'fixed')
        setType(data.type === "percentage" ? "percentage" : "fixed");

        // Handle Dates (Convert ISO string to Persian DateObject)
        if (data.startDate) {
          setStartDate(
            new DateObject({
        ...
        // Handle Plan
        if (data.plan) {
          setSpecificPlanMode(true);
          setSelectedPlan(data.plan);
        }

        // Handle Users
        if (data.users && data.users.length > 0) {
          setSpecificUserMode(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!startDate || !endDate)
      return toast.warning("لطفا بازه زمانی را مشخص کنید");
    ...
    try {
      ...
      // Assuming PUT /discount/edit or POST /discount/update
      await api.put("/discount/update", payload);
  ...
  return (
    <div className="container-fluid p-4 fade-in" style={{ maxWidth: "1400px" }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h3 className="fw-bold text-dark mb-0">ویرایش تخفیف</h3>
        <button
          onClick={() => navigate(-1)}
      ...
      <form onSubmit={handleSubmit}>
        <div className="row g-4">
          {/* --- Left Column: Preview Card (Sticky) --- */}
          <div className="col-lg-4 order-lg-last">
            ...
            >
              ...
              <div className="position-relative z-index-2 text-center text-white mb-4 mt-3">
                ...
                <h3 className="fw-bolder mb-2 text-shadow">
                  {title || "عنوان تخفیف"}
                </h3>
                <p className="text-white small opacity-75">پیش‌نمایش تغییرات</p>
              </div>
              ...
              >
                {submitting ? (
                  ...
                  <Save size={22} strokeWidth={2.5} className="text-primary" />
                )}
                <span className="text-primary">ذخیره تغییرات</span>
              </button>
            </div>
          </div>
      ...
      {/* --- Styles (Exact Copy from Create to maintain consistency) --- */}
      <style>{`
        /* --- Colors & Gradients --- */
        .preview-card {
        ...
        .custom-input { padding: 0.6rem 0.8rem; }
        ...
        .btn-percent-active {
            ...
            box-shadow: 0 4px 10px rgba(249, 115, 22, 0.3); border: none;
        }
        .btn-amount-active {
            background-color: #10b981 !important; color: white !important;
            box-shadow: 0 4px 10px rgba(16, 185, 129, 0.3); border: none;
        }
        ...
        .btn-white-glass:hover {
            background-color: #f1f5f9; color: #1d4ed8;
        ...
        .dashed-btn:hover {
            border-color: #94a3b8; background-color: #f8fafc !important;
