  ...
  if (!discount) {
    return (
      <div className="text-center mt-5">
        ...
        <button className="btn btn-secondary mt-3" onClick={() => navigate(-1)}>بازگشت</button>
      </div>
    );
  }
            ...
            >
                ...
                <div className={`card-content-wrapper ${statusInfo.isLocked ? 'blur-mode' : ''}`}>
                    <div className="position-relative z-index-2 text-center text-white mb-4 mt-3">
                        <div className="icon-box bg-white text-dark rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3 shadow-sm" style={{width: '70px', height: '70px'}}>
                            <Tag size={32} />
                        </div>
                        <h3 className="fw-bolder mb-2 text-shadow dir-ltr">{displayCode.substring(0, 10)}{displayCode.length > 10 ? '...' : ''}</h3>
                        ...
                        <div className="d-flex justify-content-between mb-3 border-bottom border-white border-opacity-25 pb-2">
                            ...
                            <span className="fw-bolder fs-4 text-white text-shadow">
                                ...
                                <small className="fs-6 ms-1 fw-light">
                                    {discount.type === 'percentage' ? '%' : (discount.type === 'amount' || discount.type === 'fixed' ? 'تومان' : '')}
                                </small>
                            </span>
                        </div>
                        <div className="d-flex justify-content-between mb-2">
                            <span className="text-white fw-light small">شروع:</span>
                            <span className="fw-bold dir-rtl small text-white">{formatDate(discount.startDate)}</span>
                        </div>
                        <div className="d-flex justify-content-between">
                            <span className="text-white fw-light small">پایان:</span>
                            <span className="fw-bold dir-rtl small text-white">{formatDate(discount.endDate)}</span>
                        </div>
                    </div>
        ...
        <div className="col-lg-8 order-lg-first">
            <div className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
                ...
                <div className="row g-4 mb-4">
                     ...
                     <div className="col-md-6 d-flex flex-column justify-content-end">
                        <div className="d-flex justify-content-between align-items-center mb-2">
                             <label className="form-label text-secondary fw-bold small ms-1 mb-0">وضعیت فعلی</label>
                        </div>
                        <div className={`d-flex align-items-center justify-content-between px-3 py-2 rounded-3 border ${statusInfo.containerClass} bg-light`}>
                             <span className="text-muted small">{statusInfo.desc}</span>
                             <span className={`badge rounded-pill px-3 py-2 ${statusInfo.badgeClass} d-flex align-items-center gap-2`}>
                                <StatusIcon size={14}/>
                                {statusInfo.label}
                             </span>
                        </div>
                     ...
                     <div className="col-md-6">
                        ...
                        <div className="input-group">
                            <span className="input-group-text bg-light border-end-0 text-muted"><Hash size={18}/></span>
                            <div className="form-control custom-input fw-bold bg-light text-dark d-flex align-items-center justify-content-between border-start-0">
                                ...
                                <button className="btn btn-sm btn-link text-muted p-0" onClick={() => handleCopy(displayCode)}>
                                    <Copy size={16}/>
                                </button>
                            </div>
                        </div>
                ...
                <div className="row g-4 mb-4">
                    ...
                    <div className="col-md-6">
                        ...
                        <div className="d-flex gap-2">
                            ...
                            <div className="d-flex bg-light rounded-3 p-1 border border-light" style={{height: '48px', minWidth: '120px'}}>
                                <div className={`w-100 d-flex align-items-center justify-content-center  rounded-2 ${discount.type === 'percentage' ? 'bg-secondary text-white' : 'text-muted'}`}>
                                    % درصد
                                </div>
                                <div className={`w-100 d-flex align-items-center justify-content-center  rounded-2 ${(discount.type === 'fixed' || discount.type === 'amount') ? 'bg-secondary text-white' : 'text-muted'}`}>
                                    $ مبلغ
                                </div>
                            </div>
                        </div>
                ...
                <div className="row g-4 mb-5">
                    ...
                    <div className="col-md-6">
                        <label className={`form-label fw-bold small ms-1 ${statusInfo.isLocked && statusInfo.lockLabel === 'منقضی شده' ? 'text-danger' : 'text-secondary'}`}>
                            تاریخ پایان
                        </label>
                        <div className={`custom-input-group bg-light d-flex align-items-center rounded-3 px-3 ${statusInfo.isLocked && statusInfo.lockLabel === 'منقضی شده' ? 'border border-danger' : ''}`}>
                            <CalendarIcon size={18} className="text-muted ms-2"/>
                ...
                <div className="mb-5">
                    <div className="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div className="form-check form-switch d-flex align-items-center gap-2 dir-rtl opacity-75">
                            ...
                            <input className="form-check-input" type="checkbox" checked={!!discount.plan} readOnly disabled />
                        </div>
                    </div>
                ...
                <div>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div className="form-check form-switch d-flex align-items-center gap-2 dir-rtl opacity-75">
                             ...
                             <input className="form-check-input" type="checkbox" checked={!!(discount.users && discount.users.length > 0)} readOnly disabled />
                        </div>
                    </div>
                        ...
                        {discount.users && discount.users.length > 0 ? (
                            <div className="d-flex flex-column gap-2">
                                {discount.users.map((user, idx) => (
                                    <div key={idx} className="d-flex align-items-center justify-content-between bg-white border px-3 py-2 rounded-3 shadow-sm hover-bg-light transition-all">
                                        <div className="d-flex align-items-center gap-3">
                                            <div className="bg-primary bg-opacity-10 text-primary rounded-circle p-2">
                                                <Users size={16} />
                                            </div>
                                            <div>
                                                <div className="fw-bold text-dark small">{user.username || "کاربر"}</div>
