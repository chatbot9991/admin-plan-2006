  ...
  return (
    <div className="container-fluid p-4 fade-in" style={{ maxWidth: "1400px" }}>
      ...
      <UserSelectorModal
        ...
        onConfirm={(users) => {
            const newUsers = users.filter(newU => !selectedUsers.some(existU => existU._id === newU._id));
      ...
      <form onSubmit={handleSubmit}>
        <div className="row g-4">
            ...
            <div className="col-lg-4 order-lg-last">
                <div className="preview-card p-4 rounded-4 shadow-lg sticky-top position-relative overflow-hidden" style={{top: '20px', minHeight: '420px'}}>
                    {/* Background Circles */}
                    <div className="bg-circle-1"></div>
                    <div className="bg-circle-2"></div>
                    
                    <div className="position-relative z-index-2 text-center text-white mb-4 mt-3">
                        <div className="icon-box bg-white text-primary rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3 shadow-sm" style={{width: '70px', height: '70px'}}>
                            <Tag size={32} />
                        </div>
                    ...
                    <div className="position-relative z-index-2 details-box bg-white bg-opacity-10 rounded-4 p-4 mb-4 backdrop-blur border border-white border-opacity-25">
                        <div className="d-flex justify-content-between mb-3 border-bottom border-white border-opacity-25 pb-2">
                            ...
                            <span className="fw-bolder fs-4 text-white text-shadow">
                                ...
                                <small className="fs-6 ms-1 fw-light">{type === 'percentage' ? '%' : 'تومان'}</small>
                            </span>
                        </div>
                        <div className="d-flex justify-content-between mb-2">
                            <span className="text-white fw-light small">شروع:</span>
                            <span className="fw-bold dir-rtl small text-white">{startDate?.format("YYYY/MM/DD") || "---"}</span>
                        </div>
                        <div className="d-flex justify-content-between">
                            <span className="text-white fw-light small">پایان:</span>
                            <span className="fw-bold dir-rtl small text-white">{endDate?.format("YYYY/MM/DD") || "---"}</span>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        disabled={loading}
                    >
                        {loading ? <span className="spinner-border spinner-border-sm text-primary"/> : <Check size={22} strokeWidth={3} className="text-primary"/>}
                ...
                <div className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
                        ...
                        <div className="col-md-6">
                            ...
                            <div className="d-flex gap-2">
                                <input 
                                    ...
                                    style={{ flex: 1 }}
                                />
                                <div className="d-flex bg-light rounded-3 p-1 border border-light" style={{height: '48px'}}>
                                    <button 
                                        type="button" 
                    ...
                    <div className="row g-4 mb-5 position-relative">
                        ...
                        <div className="col-md-6" style={{ zIndex: 1001, position: 'relative' }}>
                            <label className="form-label text-secondary fw-bold small ms-1">تاریخ پایان</label>
                            <div className="custom-input-group d-flex align-items-center rounded-3 px-3">
                                <CalendarIcon size={18} className="text-muted ms-2"/>
                                <div className="flex-grow-1 h-100">
                                    <DatePicker
                                        ...
                                        value={endDate}
                                        onChange={setEndDate}
                                        minDate={startDate || new DateObject({ calendar: persian, locale: persian_fa })}
                                        inputClass="custom-date-input"
                                        placeholder="انتخاب تاریخ"
                                        containerStyle={{ width: "100%", height: "100%" }}
                                    />
                    ...
                    <div className="mb-5 position-relative" style={{ zIndex: 1 }}>
                        <div className="d-flex justify-content-between align-items-center mb-3 px-1">
                            ...
                            <div className="form-check form-switch d-flex align-items-center gap-2 dir-rtl">
                                <label className="form-check-label fw-bold text-muted cursor-pointer" htmlFor="planSwitch">
                                    انتخاب اشتراک خاص
                                </label>
                                <input 
                        ...
                        <div className={`p-4 rounded-4 transition-all ${specificPlanMode ? 'bg-cyan-soft border border-info border-opacity-25' : 'bg-light opacity-50'}`}>
                            {specificPlanMode ? (
                                selectedPlan ? (
                    ...
                    <div className="position-relative" style={{ zIndex: 1 }}>
                        <div className="d-flex justify-content-between align-items-center mb-3 px-1">
                            ...
                            <div className="form-check form-switch d-flex align-items-center gap-2 dir-rtl">
                                <label className="form-check-label fw-bold text-muted cursor-pointer" htmlFor="userSwitch">
                                    انتخاب کاربران خاص
                                </label>
                                <input 
                                    className="form-check-input custom-toggle shadow-sm" 
                        ...
                        <div className={`p-4 rounded-4 transition-all ${specificUserMode ? 'bg-primary-soft border border-primary border-opacity-25' : 'bg-light opacity-50'}`}>
                            {specificUserMode ? (
                                <div className="animate-fade-in">
                                    {selectedUsers.length > 0 && (
                                        <div className="mb-3 d-flex flex-wrap gap-2">
                                            {selectedUsers.map(user => (
                                                <div key={user._id} className="user-pill d-flex align-items-center bg-white border border-primary border-opacity-10 text-dark px-3 py-2 rounded-pill shadow-sm">
